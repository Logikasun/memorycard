...
btn_Menu = QPushButton('Менюшкінс🎛') # кнопка повернення до основного вікна
btn_Sleep = QPushButton('Спокійно, не псіхуй') # кнопка прибирає вікно та повертає його після закінчення таймера
box_Minutes = QSpinBox() # введення кількості хвилин
box_Minutes.setValue(30)
btn_OK = QPushButton('Відповідай!😡') # кнопка відповіді
lb_Question = QLabel('') # текст питання

# Панель з варіантами:
RadioGroupBox = QGroupBox("Варіки відповідей:") 
...
AnsGroupBox = QGroupBox("Резулт тесту🤗")
...
layout_line1.addWidget(QLabel('хвилиночок')) # нам не потрібна змінна для цього напису
...
def show_result():
   ...
   RadioGroupBox.hide()
   AnsGroupBox.show()
   btn_OK.setText('Next питаннячко👀')
